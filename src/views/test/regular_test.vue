<template>
  <div class="page" style="margin-bottom: 30px">
    ...
    >
      <p
        style="padding: 5px"
        v-for="(item, index) in showCodeValue"
        :key="index"
      >
        {{ item }}
      </p>
...
export default {
  data() {
    return {
      ...
      title:
        "# 标题++++++`code`**加粗***切斜*[adsionlBlog](blog.adsionli.com) ***123***",
      specialChar: {
        bold: /(\*{2})(.+?)(\1)/gi,
        tilt: /(\*{1})(.+?)\1/gi,
        underline: /(<u>)(.+?)(<\/u>)/gi,
  ...
  mounted() {
    ...
    this.axios.get(this.mdPath[1]).then(res => {
      this.tableRegular.data = res.data.replace(/\r/gmi, '');
      this.tableRegular.showDataHtml = this.tableRegular.data.split('\n')
      this.matchingPattern.resetData();
      this.matchingPattern.handle(this.tableRegular.data);
    }).catch((error) => {
      console.log(error);
    })
  },
  methods: {
    testTitle() {
      ...
      if (reg.test(this.title)) {
        let count = this.title.match(/(?<titleCount>^#{1,6})(\s{1,})(.+?)/)
          .groups.titleCount.length;
    ...
    testUnderline() {
      ...
      this.testData = this.testData.replace(
        reg,
        `<font class='underline_char'>$2</font>`
      );
    },
    testInlineCode() {
      ...
      this.testData = this.testData.replace(
        reg,
        `<span class='inlineCode_char'>$2</span>`
      );
    },
    testBold() {
    ...
    testHighlight() {
      ...
      this.testData = this.testData.replace(
        reg,
        `<font class='highlight_char'>$2</font>`
      );
    },
    testTilt() {
    ...
    testDeleteLine() {
      ...
      this.testData = this.testData.replace(
        reg,
        `<del class='deleteline_char'>$2</del>`
      );
    },
    testHypersLink() {
      let reg = new RegExp(/(\[(.+?)\])(\((.+?)\))/gi);
      console.log(reg.exec(this.testData));
      for (let value of this.testData.matchAll(
        /(\[(?<name>.+?)\])(\((?<url>.+?)\))/gi
      )) {
        console.log(value);
      }
      this.testData = this.testData.replace(
        reg,
        `<a href='$4' rel='noopener noreferrer' target='_blank'>$2</a>`
      );
    },
  },
};
