...
WebPack可以看做是**模块打包机**：它做的事情是，分析你的项目结构，找到JavaScript模块以及其它的一些浏览器不能直接运行的拓展语言（Scss，TypeScript等），并将其打包为合适的格式以供浏览器使用。cn
...
​		webpack第一阶段学习到这里就结束了，在第一阶段学习中，终于知道了webpack的作用，以及它的使用方法，同时还学会了如何去配置browserslist和postcss，平时这些都很少会去了解因为基本都是用默认了，现在学会了如何配置之后，就可以针对很多不同的需求去进行配置，来完成各个不同平台的适配，更加的方便。
